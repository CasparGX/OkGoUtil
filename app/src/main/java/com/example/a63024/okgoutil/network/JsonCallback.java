...
public abstract class JsonCallback<T> extends AbsCallback<T> {
    ...
    public JsonCallback(String secret) {
        this.secret = secret;
    }
    ...
    public T convertSuccess(Response response) throws Exception {
        ...
        //有数据类型，表示有data
        Gson gson = new Gson();
        //String jsonStr =  gson.toJson(Convert.fromJson(jsonReader, type));
        String jsonStr =  gson.toJson(Convert.fromJson(jsonReader, new TypeToken<LzyResponse<T>>() {}.getType()));

        LzyResponse lzyResponse = new LzyResponse();


        JSONObject jsonObj = new JSONObject(jsonStr);
        lzyResponse.result  = Boolean.parseBoolean(jsonObj.getString("result"));


        if (lzyResponse.result) {
            //有数据类型，表示有data
            response.close();
            String obj = UrlApi.decryptResult(jsonObj.optString("data", ""),secret);
            Log.e("+++数据返回打印++++", obj);
            T result = Convert.fromJson(obj, type);//gson.fromJson(obj, new TypeToken<T>() {}.getType());
            return result;
        }else{
            String code = jsonObj.getString("error_code");
                if (code.equals("-1") ) {
                    throw new IllegalStateException(code + "#" + jsonObj.getString("error_msg"));
